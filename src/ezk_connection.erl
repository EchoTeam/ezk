...
start_link(Servers, MaxConnectionAttempts) ->
    ?LOG(1, "Connection: start_link called with args: ~p", [[Servers, MaxConnectionAttempts]]),
    gen_server:start_link(?MODULE, [Servers, MaxConnectionAttempts], []).
...
die(ConnectionPID, Reason) ->
    ?LOG(3, "Connection: die with reason: ~p", [Reason]),
...
n_create(ConnectionPID, Path, Data, Receiver, Tag) ->
    gen_server:cast(ConnectionPID, {nbcommand, {create, Path, Data, [], [undef]},
				    Receiver, Tag}).
%% Type = e | s | es (stands for ephemeral, sequenced or both)
create(ConnectionPID, Path, Data, Type) ->
    gen_server:call(ConnectionPID, {command, {create, Path, Data, Type, [undef]}}).
n_create(ConnectionPID, Path, Data, Type, Receiver, Tag) ->
    gen_server:cast(ConnectionPID, {nbcommand, {create, Path, Data, Type, [undef]},
				    Receiver, Tag}).
...
%% where Per = r | w | c | d | a
create(ConnectionPID, Path, Data, Type, Acls)  ->
    gen_server:call(ConnectionPID, {command, {create, Path, Data, Type, Acls}}).
n_create(ConnectionPID, Path, Data, Type, Acls, Receiver, Tag)  ->
    gen_server:cast(ConnectionPID, {nbcommand, {create, Path, Data, Type, Acls},
				    Receiver, Tag}).
...
%% Deletes a ZK_Node and all its children.
%% Reply = Path where Path = String
%% If deleting some nodes violates the acl
%% or gets other errors the function tries the
%% other nodes before giving the error back, so a
%% maximum number of nodes is deleted.
%% a) List children of Node. If it has none everything is all right.
%% b) If he has some: kill them and their Children rekursively.
%% c) Kill the Node with delete
delete_all(ConnectionPID, Path) ->
    ?LOG(3, "Connection: Delete All: Trying to Erase ~s", [Path]),
    Childs = ls(ConnectionPID, Path),
    case Childs of
        {ok, []} ->
	    ?LOG(3, "Connection: Killing ~s", [Path]),
	    delete(ConnectionPID, Path);
	{ok, ListOfChilds} ->
	    ?LOG(3, "Connection: Delete All: List of Childs: ~s", [ListOfChilds]),
            case Path of
		"/" ->
		    lists:map(fun(A) ->
...
init([Servers, MaxConnectionAttempts]) ->
    ?LOG(1, "Connection: Servers: ~p", [Servers]),
    random:seed(erlang:now()),
    process_flag(trap_exit, true),
    K = n_init_attempts(Servers, MaxConnectionAttempts),
    ?LOG(1, "Connection: Connection status: ~p", [K]),
    K.

n_init_attempts(_Servers, 0) ->
    {stop, {error, no_server_reached}};
n_init_attempts(Servers, N) ->
    ?LOG(1, "Connection: Connect init. incoming args: ~p", [Servers]),
    WhichServer = random:uniform(length(Servers)),
    ?LOG(0, "Connection: Choose server ~p", [WhichServer]),
...
%% a) Save an entry in the watchtable (key: {Type, Path})
...
handle_call({watchcommand, {Command, CommandW, Path, {WType, WO, WM}}}, From, State) ->
	...
	_Else ->
	    ?LOG(3, " Connection: There is a watcher already with the specified path/type."),
...
handle_info(heartbeat, State) ->
	...
	0 ->
            ?LOG(4, "Connection: Send a Heartbeat"),
...
send_watch_events_and_erase_receivers(Table, Receivers, Path, Type, SyncCon) ->
    case Receivers of
	[] ->
	...
	[{Key, WatchOwner, WatchMessage}|T] ->
	    ...
	    Message = {WatchMessage, {Path, Type, SyncCon}},
            ?LOG(3, "Connection: The Message is  ~w", [Message]),
            WatchOwner ! Message,
	    send_watch_events_and_erase_receivers(Table, T, Path, Type, SyncCon)
    end.
...
handle_typed_incomming_message({heartbeat,_HeartBeat}, State) ->
    ?LOG(4, "Connection: Got a Heartbeat"),
...
handle_typed_incomming_message({watchevent, Payload}, State) ->
    ...
    {Type, Path, SyncCon} = ezk_packet_2_message:get_watch_data(Payload),
    Watchtable = State#cstate.watchtable,
    ?LOG(3, "Connection: Got the data of the watchevent: ~w", [{Type, Path, SyncCon}]),
    Receivers = get_receivers(Type, Path, Watchtable),
    ?LOG(3, "Connection: Receivers are: ~w", [Receivers]),
    ok = send_watch_events_and_erase_receivers(Watchtable, Receivers, Path,
					       Type, SyncCon),
    ?LOG(3, "Connection: The first element in the watch table: ~p", [ets:first(Watchtable)]),
